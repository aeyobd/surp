...
Fig.~\ref{fig:y_cc} shows the integrated yields of massive star models from the literature. At solar metallicity, \citepos{sukhbold+16} C yield is $\approx0.5$ dex higher than \cite[non-rotating]{LC18}, likely driven by much stronger (C-enhanced) mass loss in \citet{sukhbold+16} for stars more massive than $\gtrsim40\,\Mo$.
...
As shown in Fig.~\ref{fig:y_cc}, the fiducial parameters imply a yield that agrees reasonably well with the model predictions at low $Z$ and a metallicity dependence in between that of \citeauthor{LC18}'s (\citeyear{LC18}, LC18) non-rotation and $v_{0}=300\,{\rm km\,s^{-1}}$ models.
...
As we will demonstrate in Section~\ref{sec:agb_results} below, the final locus of the \caah\ trend (the end points of the curves in the left panel) depends mostly on the metallicity dependence of the total C yield. The \caafe{} trend instead reveals the nature of delayed C production.
...
We adopt the following priors for each model parameter: $\aagb \sim N(1, 1)$, $\zcc \sim N(0, 10^{-3})$, and $\yl \sim N(2\times10^{-3}, 1\times10^{-3})$, where $N(\mu, \sigma)$ is a Gaussian distribution with mean $\mu$ and standard deviation $\sigma$. 
...
We thank Falk Herwig and Jennifer Johnson for helpful comments and discussion.
