...
\footnotetext{In our model, the mass-lifetime relation is
...
Hot bottom burning and third dredge-up result in mass and metallicity dependent C yields. 
...
Metal poor stars dredge up more material due to the decreased power of the CNO cycle, resulting in increasing carbon yields with decreasing metalicity \citep{ventura+13}.

For our models to match observations, we find that need to uniformly amplify these yield tables.
\begin{equation} \label{eq:alpha}
        \Ycagb \rightarrow \alpha_{\rm agb}\ \Ycagb.
\end{equation}
We use \cxi\ table, with $\alpha_{\rm agb}=\cfactor$, as the fiducial \agb\ yield.
Variations in models are due to different treatments of reaction rates, convection, and mass-loss.
Table~\ref{tab:agb} contains the stellar masses and metallicites each yield set contains, the approximate solar C yield $\Ycagb(\Zo)$, the metallicity dependence at solar $\zeta^{\rm agb} = d\Ycagb/dZ$, and the required factor $\alpha$ to reach 20\% \agb\ C production.
