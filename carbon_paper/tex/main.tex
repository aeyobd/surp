...
\strike{We then infer constraints on yield parameters in Section~6.}
\strike{In Section~7, we compare our model to observations of halo stars and extragalactic \Hii{} regions from the literature. }
We \strike{summarize}\add{discuss} our conclusions in Section~\ref{sec:conclusions}. 
...
Fig.~\ref{fig:subgiants} shows the [C/Mg] abundance ratios of our subgiant sample as a function of [Mg/H] and [Mg/Fe].\footnotemark{} In this sample, [C/Mg] increases with [Mg/H] and decreases with [Mg/Fe] at fixed [Mg/H].
\add{
Some previous studies \citep[e.g.,][]{tinsley79, KKU11, KKL20} have focused primarily on the [C/Fe]-[Fe/H] ratio, interpreting an increase or decrease in [C/Fe] with [Fe/H] as an indication of the timescales of C production by a single stellar population relative to Fe. However, we show in Section~\ref{sec:n_and_fe} below that the metallicity dependence of C yields produces similar effects as delayed enrichment sources like SNe Ia and AGB stars \citep[see also][]{romano22}. Our inclusion of Mg allows us to more confidently pin down the characteristic delay-time of C production by AGB stars, and by extension the metallicity-dependence of its yield, by connecting C abundances to the decline in [Mg/Fe] with time.
}
...
\begin{figure*}
    \centering
    \includegraphics[]{y_agb_threepanel_shifted.pdf}

    \caption[]{
        \add{AGB yields as functions of mass (left), metallicity (middle), and delay-time (right). \textbf{Left:} The net stellar C yield for the \fruity{} AGB yield table as a function of initial mass and colour-coded by metallicity.
        }
        \textbf{\replace{Left}{Middle}:} The integrated C yield from AGB stars, $\Ycagb$, as a function of metallicity for each yield table,  calculated $10\,{\rm Gyr}$ after a stellar population forms. (Note that the shape of the integrated yields arises from our linear interpolation of yields in $Z$. Changing to linear interpolation in $\log Z$ does not substantially affect our results.)
...
We use the same tables as \citet{james+23}, except that we have swapped \citet{karakas10} for \citet{pignatari+16}. (See Appendix~\ref{sec:extra_agb} for the implementation of these yields \add{and discussion of the differences between AGB yield models}.)



\strike{Fig.~\ref{fig:y_agb} compares the C yields for these four AGB models.
...
\add{We show only the \fruity{} yields here because each of the yield tables we use are in qualitative agreement regarding the trends in mass and metallicity (see Appendix~\ref{sec:extra_agb}).
C production peaks between masses of $\sim$ 2--4 $\Mo$. As metallicity increases, the net yield decreases and the mass of peak C production increases slightly. As a result, most AGB models predict that C production becomes less efficient but more rapid with increasing metallicity.
}
...
$
where  $l=0.08\,\Mo$ and $u = 100\,\Mo$ are the assumed minimum and maximum mass of star formation. We assume a \citet{larson74} main sequence mass-lifetime-relationship. 
\add{Although there are other, more recent forms of the mass-lifetime relation available in the literature \citep[e.g.,][]{hurley+2000, vincenzo+2016}, the updates have been focused primarily on the low- and high-mass ends of the IMF, leaving the AGB star mass range (${\sim}1$--$8\,\Mo$) largely unaffected. The highest mass stars live sufficiently short lives that their enrichment can be approximated as instantaneous for most purposes \citep[see discussion in, e.g.,][]{WAF17}, while the lowest mass stars have lifetimes larger than the age of the Universe and therefore do not meaningfully influence chemical evolution. As a consequence, the exact choice of mass-lifetime relation has little to no impact on chemical evolution. 
}
We take the total stellar lifetime (through post-AGB) to be $1.1\times$ the main sequence lifetime. Thus, the mass $m_\textrm{post-AGB}$ is slightly larger than the main-sequence turn-off mass.


The IMF-averaged yields computed from the literature tables differ in normalization and metallicity dependence.  
The \replace{left}{middle} panel of Fig.~\ref{fig:agb_yields} shows IMF-averaged C yields for each AGB model as a function of metallicity, computed at time $\tau = 10$ Gyr after the formation of a single stellar population.
...
The right panel of Fig.~\ref{fig:agb_yields} shows the time-dependence of C production by AGB stars in a single stellar population of metallicity ${\rm [M/H]} = -0.1$ as a function of age. 
...
As our fiducial AGB yield, we use the \fruity\ table with \strike{$\aagb=2.56$ ($\fagb=0.3$), balancing MW and gas-phase constraints (see Section~\ref{sec:results} and Section 6}\add{$\aagb=\cfactor$ or $\fagb=0.25$ based on a best-fit computed in Appendix~\ref{sec:extra_mcmc}.} 
...
We approximate massive star enrichment as occurring instantaneously after a star formation event. Under this approximation, the yield is a constant  of proportionality between the metal production rate and the star formation rate (SFR):
\begin{equation}
...
Fig.~\ref{fig:agb_yields} also shows the IMF-averaged AGB star yields from the \fruity\ model \strike{(from Fig.~\ref{fig:agb_yields})}. Especially at $Z\approx \Zo$, most CCSN models dominate AGB C production. 
...
\add{Given the wide range of predictions for CCSN C yields, w}\strike{W}e choose to adopt a linear yield model instead of a particular literature yield table\strike{, given the wide range of predictions for CCSN C}. We parameterize the yield as
...
\add{$\yo$ can be thought of as the yield of primary CCSN C (produced without dependence on metallicity) and $\zcc$ as the amount of secondary CCSN C (produced with a linear dependence on metallicity). }
For our fiducial model, we adopt $\yo =$ \strike{18.7} ${19.7}\times 10^{-4}$ and $\zcc = $\strike{7.9} ${8.56} \times 10^{-4}$, leading to reasonable agreement with the \caah{}, \caafe{}, and gas-phase abundance relations \add{(see Section~\ref{sec:results} and Appendix~\ref{sec:extra_gas})}. \strike{We present best-fitting models to subgiants only in Section~6.  
We have also explored higher-order prescriptions (i.e., second-, third-, and fourth-order polynomials), finding improved agreement with the observed abundances with each successive free parameter.
...
As shown in Fig.~\ref{fig:y_cc}, the fiducial parameters imply a yield that agrees reasonably well with the model predictions at low $Z$ and a metallicity dependence in between that of}\red{ \citeauthor{LC18}'s (\citeyear{LC18}, LC18) }\strike{non-rotation and $v_{0}=300\,{\rm km\,s^{-1}}$ models.}
\add{
This linear prescription is able to capture all of the effects necessary for the purposes of this paper, despite its simplicity. We have repeated our analysis with higher order prescriptions (e.g., second-, third-, and fourth-order polynomials) and found that these additional degrees of freedom do not change our conclusions. We find a statistically better reproduction of the observed C abundances with each successive free parameter, suggesting that C production is indeed more complex than a linear metallicity dependence. However, the additional complexity is not needed for our purposes.  
}
...
\begin{figure}
   ...
   The light blue line denotes $\Ycagb$ from the \fruity{} AGB star model for comparison (see Fig.~\ref{fig:agb_yields}). All models include wind yields. 
}
    \label{fig:y_cc}
\end{figure}
...
In these models, the Galaxy is divided into 200 rings, each representing a single 100\,pc zone. Each ring has a separate \replace{stellar population and gas supply}{star formation rate, gas reservoir, gas inflow, and possible gas outflow}. We initially assume the inside-out SFH from \JJ, where the star formation surface density $\dot{\Sigma}_\star$ is given by 
...
Compared to \citet{james+21, james+24}, our metallicity gradient is flatter within 5 kpc\add{, since the outflow rates go to zero near this radius}\strike{(and our $\eta\to0$ at similar radius)}.
If we change our assumed yield scale, the values of $\eta$ will re-scale accordingly to maintain observationally-consistent chemical trends. Following \JJ, we neglect radial gas flows \add{(see discussion below)}. 
...
Our AGB N yield is scaled from \citep[eq. 14]{james+23}, based on the differences in the assumed solar N/O ratio and the median [N/Mg] or [N/O] value of each sample.  \strike{Following {james+23}, we use a $t^{-1.1}$  delay time distribution (DTD) for SN Ia with a minimum delay-time of 150 Myr, based on comparisons of the SN Ia rate as a function of redshift and the cosmic SFR (e.g., {maoz+mannucci12}).}
\add{Based on the finding by \citet{dubay+24} that GCE models favour extended delay-time distributions (DTD), we assume an event rate that is initially constant with a minimum delay time of 40 Myr, which then transitions to a $t^{-1.1}$ time-dependence after 1 Gyr. This DTD is similar to \citepos{greggio2005}  `WIDE DD' model.}
We assume $1.1\times10^{-3}$ SN Ia events per unit mass of star formation  with a mean yield of $0.710\,\Mo$ of newly produced Fe per supernova \citep{david_fe}.
We use a \citet{kroupa01} initial mass function (IMF).

\add{
A known problem for CCSN yield models is that O tends to be overproduced and Mg underproduced, i.e. the O-Mg problem \citep[e.g.,][]{emily+21}. For simplicity, we adopt metallicity-independent values of O and Mg based on the observed APOGEE abundance trends from \citet{david_fe} \citep[see also][]{andrews+17, weinberg+19}. 
}

\begin{figure*}
\centering
\includegraphics{figures/all_the_tracks_shifted.pdf}
\caption[]{
...
\add{
%Galactic winds may not describe the Milky Way gas dynamics. Instead, winds driven by stellar feedback may fall back onto the Milky Way in Galactic fountains or gas may flow between regions of the galaxy in \textit{radial gas flows} \citep[e.g.][]{spitoni+2008, oppenheimer+dave2008}. While these phenomena appear to be substantially different from our assumed outflows, the details of where gas moves in the Milky Way only marginally changes the effective chemical evolution. 
%Galactic winds are equivalent to the scenario where gas both leaves the galaxy and is accreted through enriched winds. However, as the metallicity of the infalling gas is typically much lower than the interstellar medium, the changes to chemical evolution are relatively small. Specifically \citet[section 5.2]{WAF17} discuss how chemical evolution with metal-enriched inflows with a metallicity $\approx 0.2\Zo$ would be equivalent to increasing the yield of elements by $\approx 20\%$ for a zone which reaches near solar metallicity. Indeed, suspected infalling high-velocity clouds typically have metallicities $Z\sim0.2\Zo$ \citep{tripp+2003}. 
%To avoid unnecessary complexity in our model, we adopt simple outflowing winds with pristine infall in this work.


Many GCE models with a variety of motivations omit mass loading in Galactic winds (i.e., $\eta=0$, e.g., \citealt{spitoni19, spitoni20, spitoni21, palla+20}).
This choice is based on hydrodynamic simulations in which ejected gas re-accretes to a similar location on short timescales \citep[e.g.,][]{melioli+2008, melioli+2009, spitoni+2008, spitoni+2009, oppenheimer+dave2008, hopkins+2023}. However, we emphasize that this outcome ultimately arises as a consequence of the feedback prescription built into the simulation, which is not only highly uncertain but also dependent on the implementation of the simulation itself (e.g., \citealt{li+2020, hu+2023}; see also the reviews by \citealt{veilleux+2020, thompson+heckman2024}). 
Significant mass-loading indeed arises in simulations with more energetic feedback \citep[e.g.,][]{brook+2011, gutcke+2017, nelson+2019, peschken+2021, kopenhafer+2023}. 
Following the models without outflows in the literature, we would instead use the radial flows of gas towards the Galactic centre \citep[e.g.,][]{wong+2004, diteodoro+peek2021}. 
This process has seen much more extensive use in GCE models of the MW than outflows \citep[e.g.,][]{lacey+fall1985, portinary+chiosi2000, spitoni+matteucci2011, bilitewski+schonrich2012, pezzulli+fraternali2016}. 
However, both fulfill the same purpose in our models, which is to provide freedom to adjust the radial metallicity gradient that arises in the ISM \citep[e.g.,][]{grisoni+2018, palla+20, james2025}. We therefore expect similar outcomes with either choice, since the relevant free parameters would be chosen such that the model reproduces the same measurement of $\nabla \mathrm{[O/H]}$. 
}
...
As we will demonstrate in Section~\ref{sec:agb_results} below, the final locus of the \caah\ trend (the end points of the curves in the left panel) depends mostly on the metallicity dependence of the total C yield. The \caafe{} trend instead reveals the nature of delayed C production. \add{Stars with lower [Mg/Fe] values have been enriched more by SN Ia and, consequently, should also contain more delayed C from AGB stars. }
...
\subsection{\replace{Nitrogen}{Iron and Nitrogen}} \label{sec:n_and_fe}
\note{Swapped Section 5.2 and 5.3.}

\add{
Fig.~\ref{fig:sims_fe} compares the low-$\alpha$ [C/Fe]-[Fe/H] trends for different C CCSN yield slopes and AGB yield fractions. This trend is predominantly determined by the metallicity dependence of the C yield, similar to the \caah{} trend. Increasing the AGB star contribution ($\fagb$) and the metallicity-dependence of the CCSN yield ($\zcc$) simultaneously leaves the [C/Fe]-[Fe/H] trend unchanged. 
$\fagb$ and $\zcc$ therefore cannot be inferred confidently from the [C/Fe]-[Fe/H] trend alone. This degeneracy between delayed production and metallicity dependent yields is our motivation for including Mg abundances in our sample. This additional information allows us to isolate these two effects more confidently through comparisons to the [Mg/Fe] ratio.
}

\begin{figure}
    \centering
    \includegraphics{figures/sims_fe.pdf}
    \caption{\red{Similar to the left panel of Fig.~\ref{fig:zeta_f} except using Fe as the reference element instead of Mg. Increasing the fraction of AGB production of C similarly affects the [C/Fe]-[Fe/H] trend as decreasing the metallicity dependence of CCSN C production. The [C/Fe]-[Fe/H] trend therefore cannot distinguish metallicity-dependent CCSN yields from delayed AGB C-production, prompting our inclusion of Mg abundances in our sample.}}
    \label{fig:sims_fe}
\end{figure}
...
\begin{figure}
    \includegraphics{figures/sims_agb.pdf}
    
    \caption[]{
        The best-fitting yield combination for each AGB table and the \fruity{} table with a mass shift of 0.7. We fit $\aagb$, $\yo$, and $\zeta$ using the method in Appendix~\ref{sec:extra_mcmc}.
        \note{Other panels moved to appendix}
    }
    \label{fig:sims_agb}
\end{figure}


All AGB yield tables predict a similarly shaped trend in the \caafe{} abundance space. \strike{The left panel of }Fig.~\ref{fig:sims_agb} shows the best-fitting yield combinations for each AGB table we consider (solving for $\aagb$, $\yo$, and $\zcc$ in Appendix \ref{sec:extra_mcmc}\strike{; yield parameters in Table~\ref{tab:model_parameters}}). Although the individual DTDs of each AGB table appear substantially different (Fig.~\ref{fig:agb_yields}), the predicted abundance trends do not show substantial differences. 
This outcome arises because each of the AGB models predicts characteristic delay times for C production of order ${\sim}1$ Gyr, similar to the SN Ia production of Fe (see Fig.~\ref{fig:agb_yields}), leading to a quasi-linear [C/Mg]-[Mg/Fe] relation in all cases. 

However, all AGB tables predict a slightly concave down trend because C production by single stellar populations slows down significantly after a few Gyr, while Fe production through SNe Ia continues up to ${\sim}10$ Gyr (see discussion in Section~\ref{sec:agb}). Our subgiant sample does not exhibit this level of concavity. These discrepancies are significant because Fig.~\ref{fig:sims_agb} plots the mean trend, which is precisely determined through the large sample size. One of the largest discrepancies between our models and data in the left panel is that \aton{} and \monash{} predict an almost flat \caafe{} trend for ${\rm [Mg/Fe]} \lesssim 0.1$.
...
We explore this possibility further in} \strike{the middle panel of} Fig.~\ref{fig:sims_agb}, which \add{also shows a variation} of our fiducial model in which we artificially multiply the AGB star progenitor mass \add{a factor of} \add{0.7}.
\strike{When the AGB C production is weighted towards more massive AGB stars, the DTD becomes more rapid, leading to higher [C/Mg] at high [Mg/Fe]. On the other hand, s}Shifting the C production toward lower mass progenitors makes the DTD more extended\add{, comparable to SN Ia. As a result, the mass-shifted \fruity{} model has a more linear \caafe{} trend, better matching the observed abundance pattern. We explore more mass-shift variations in Appendix~\ref{sec:extra_models}.}
...
However, recent GCE models indicate that abundance trends favour\strike{, if anything,} an extended DTD  \add{as used here} (\citealt{palicio+2024} and \citealt{dubay+24}).




\subsection{The best-fitting yield combinations}
\red{(Note: this section is new, and the original section 6 is moved now to the appendix)}

\add{
To derive more quantitative constraints on the origin of carbon, Appendix~\ref{sec:extra_mcmc} presents a yield-fitting framework where we use Monte-Carlo Markov Chains to find the combination of CCSN and AGB yields that match the C abundance trends as binned functions of [Mg/H] and [Mg/Fe]. 
Fig.~\ref{fig:mcmc_fagb} plots the best-fitting $\fagb$ for a variety of models, including alternate AGB yields, star formation histories, and yield scales. \aton{} is the sole outlier with $\fagb\approx0$ because the AGB yield itself is nearly zero at solar metallicity (see Fig.~\ref{fig:y_agb}).
In all other cases, the best-fitting value of $\fagb$ falls between 0.15 and 0.3 (the shaded region). 

Appendix~\ref{sec:extra_models} explores additional models varying the yield scale, star formation history, and type Ia supernovae scale. Changes to the star formation history or yield scale only slightly change the abundance trends. Instead, the type Ia SN Fe production directly impacts the derived carbon abundance, with derived $\fagb$ potentially reaching 50\% for high SN Ia Fe yields. However for our best-fitting (mass-shifted) AGB model, a reasonable increase in the type Ia supernovae rate increases the derived $\fagb$ from 20\% to 25\%. 
}
...
\begin{figure}
    ...
    \caption[]{
     The best-fitting $\fagb$ from the MCMC analysis samples for several model (see Appendix~\ref{sec:extra_mcmc}). \add{We include results for different AGB tables (\fruity{}, \aton{}, \monash{}, \nugrid{}), a model where the \fruity{} is artifically shifted in progeniter mass by a factor of 0.7, a model adopting a different, twoinfall star formation history (from Appendix~\ref{sec:extra_models}), and a model where all yields are doubled and outflows adjusted according to Eq.~\ref{eq:mass_loading}.} The shaded region represents our suggested $\fagb$ for the Milky Way's thin disk at solar metallicity.
...
From the combined constraints on the [C/Mg] trend with [Mg/H] and [Mg/Fe], we have inferred possible combinations of yields that reproduce thin-disk abundance trends. In all cases, we find that $\Yct / \Ymg \approx$ \strike{4.0} $\red{4.2} \pm 0.2$ at solar metallicity, with total C production increasing with increasing metallicity. We also show that low-intermediate mass AGB stars likely produce a fraction of between \add{0.15 and 0.30} of solar C, across GCE assumptions and AGB yield tables.
...
\section{NuGrid yield tables \add{and comparison of AGB yields}} \label{sec:extra_agb}




In this Section, we present updated calculations of the \citet{battino+19, battino+21} yield tables and describe their incorporation into \VICE\add{, and compare AGB yields in more detail}. These models use the NuGrid MPPNP post-processing toolkit on models run using the \textsc{mesa} stellar evolution code \citep{mesa}.
...
\add{
Fig.~\ref{fig:y_agb} compares each AGB yield table as a function of both mass and metallicity. 
In general, all tables have a similar qualitative shape. AGB C production peaks at initial masses of $\sim$ 2--4 $\Mo$, and the peak mass increases slightly with metallicity. At higher metallicities, intermediate mass stars ($M \gtrsim 5\Mo$) start to deplete C and most stars produce less C in general. The strongest outlier is the \aton{} model, which predicts that most AGB stars deplete C at solar metallicity. Otherwise, it is reasonable to expect these models of C AGB production to predict similar GCE abundance trends.
}

\begin{figure*}
    \centering
 	    \includegraphics[scale=1]{agb_yields.pdf}
        \caption[]{The net fractional C yield from AGB stars as a function of initial stellar mass and metallicity \add{(similar to the left panel of Fig.~\ref{fig:agb_yields})}. Each panel represents yields from one of four AGB studies from the literature: \fruity{}, \aton{}, \monash{}, \nugrid{} (see Section \ref{sec:agb}).  The black dotted line shows $\Ycagb=0$ for reference. 
        }
...
\section{Additional results}\label{sec:extra_models}
\add{
In this Appendix, we explore additional models not included in the main text. We test modifications to the AGB yield table, the star formation history, and the yield scale. In most cases, the results do not change too substantially.
}

\subsection{Model parameters and measurement scatter} \label{sec:extra_scatter}


\begin{table}
	\centering
        \caption[]{Description of the yields used for models presented in this
        paper (see Sections \ref{sec:agb}, \ref{sec:vice}, and \ref{sec:results}
        for details).
        \note{Models updated to new fiducial model.}
        }
	...
	\begin{tabular}{l l l l l l l l l}
		\hline
            ...
            \hline
            fiducial & 2.14 & FRUITY & 19.7 & 8.6 \\
            $\zcc=0.0004$ & 2.14 & FRUITY & 19.7 & 4.3 \\
            $\zcc=0.0012$ & 2.14 & FRUITY & 19.7 & 12.8 \\
            $\fagb = 0$ & 0.0 & FRUITY & 27.8 & 8.6 \\
            $\fagb = 0.5$ & 4.11 & FRUITY & 12.8 & 8.6 \\
            lower $\fagb$, $\zcc$ & 0 & FRUITY & 27.8 & 4.3 \\
            higher $\fagb$, $\zcc$ & 4.11 & FRUITY & 12.8 & 12.8 \\
            \aton{} & 1.53 & ATON & 25.1 &  14.8 \\
            \monash{} & 2.04 & Monash & 19.9 & 15.9 \\
            \nugrid{} & 0.82 & NuGrid & 19.7 & 4.7 \\
            \fruity{} shifted & 1.48 & FRUITY($M/0.7$) & 22.1 & 5.3 \\
            mass shift 0.5 & 2.62 & FRUITY($M/0.5$)  & 22.2 & 4.3 \\
            mass shift 0.7 & 1.92 & FRUITY($M/0.7$) & 20.7 & 6.3 \\
            mass shift 1.5 & 2.43 & FRUITY($M/1.5$) & 22.6 & 4.3 \\
		\hline
	\end{tabular}
\end{table}
...
\subsection{AGB yield mass shifts}
\note{Originally in main text.}

\add{In the main text, 
we mention that shifting the \fruity{} table artificially in progenitor mass improves agreement with observed abundance trends. Here, we explore the effect on abundance trends of shifting the \fruity{} table intial masses by different factors. 
}

{\strike{These discrepancies in the [C/Mg]-[Mg/Fe] trend indicate that C production by single stellar populations may be more extended than our yield models predict.
We explore this possibility further in }Fig.~\ref{fig:sims_degens}\strike{, which} shows variations of our fiducial model in which we artificially multiply the AGB star progenitor mass by factors of $0.5$, $0.7$, and $1.5$.
...
Adjusting our SN Ia DTD to a prescription that declines more rapidly with stellar population age would also linearize the [C/Mg]-[Mg/Fe] relation.
However, recent GCE models indicate that abundance trends favour\strike{, if anything, a more extended}\add{the more extended DTD we adopt here}\strike{ DTD than SN surveys have found }(see discussion in \citealt{palicio+2024} and \citealt{dubay+24}).

\subsection{Star formation history and yield scale}
\note{Originally in main text.}

\begin{figure*}
    \includegraphics{sims_agb_mass_sfh.pdf}
    
    \caption[]{
        Similar to Fig.~\ref{fig:zeta_f}, but focusing on [C/Mg] versus [$\alpha$/Fe]. \add{Models where the \fruity{} AGB yields are artificially shifted by different multiplicative factors in progenitor mass, but maintaining the same $\fagb$. }
        \strike{
        Similar to Fig.~\ref{fig:zeta_f}, but focusing on [C/Mg] versus [$\alpha$/Fe]. \add{Models where the \fruity{} AGB yields are artificially shifted by different multiplicative factors in progenitor mass, but maintaining the same $\fagb$. }
        {\bf Middle:} 
        }
        {\bf Right:} Models with alternative star formation histories
        (Eqs.~\ref{eq:lateburst}, \ref{eq:twoinfall}), and a model where all yields are doubled (with mass-loading adjusted accordingly).
        }
...
\end{equation}
where $A=1.5$ represents the amplitude of the burst, $t_\text{burst}=10.8$\,Gyr is the time where the burst is strongest, and $\sigma_\text{burst}=1$\,Gyr is the width of the burst.


In the right-hand panel of Fig.~\ref{fig:sims_degens}, we consider a twoinfall evolutionary history \citep[e.g.,][]{chiappini+97,spitoni19, spitoni21, spitoni+23}. This prescription posits that the majority of the stellar mass of the Galactic disk was built up during two events of substantial metal-poor accretion separated in time. The twoinfall scenario offers a natural explanation \citep{spitoni19, spitoni21} for the bimodal distribution of [$\alpha$/Fe] ratios observed in the MW \citep[e.g.,][]{hayden+15}. However, the twoinfall model has recently faced challenges when confronted with large catalogues of stellar age measurements \citep{dubay+25}. 
For this, we switch \VICE{} to `infall mode' and let it compute the SFR from a specified accretion history, in contrast to the other models where we specify the SFH (see discussion in Section~\ref{sec:vice}). Our accretion history, following \citet{dubay+24}, is given by
...
The right panel of Fig.~\ref{fig:sims_degens} compares the fiducial SFH with these lateburst and twoinfall variations. The differences in the \caafe\ trend are small. This model predicts a different distribution in [Mg/Fe] as expected \citep[e.g.,][]{JW20} but not shown here.
...
\section{Yield Inference} \label{sec:extra_mcmc}
\note{Old section 6 moved here.}
...
\begin{enumerate}
    ...
    \item $\yo$: Constant CCSN yield
    \item $\zcc$: Linear CCSN yield ($y = \zcc (Z/\Zo)$),
\end{enumerate}
where $\yo$ represents the C yield at low metallicity. \add{In detail, the $\zcc$ includes a constant CCSN yield $\Ycc=\zcc$ to avoid from the abundance becoming negative, however this additional parameter is subtracted out again in our analysis}


\note{We remove the $\sigma_{\rm sys}$ model for simplicity below, and replace $y_{\rm low}$ with $y_0$.}
...
\end{equation}
We calculate the total variance in each bin from the standard error on the mean of the observations in a bin $\sigma_{b, \rm obs}$, the standard error on the mean for each model component $i$, \add{and} $\sigma_{b, i,\rm model}$\strike{, and a {\it systematic} uncertainty term $\sigma_{\rm sys}$}. Altogether, 
\begin{equation}
    \sigma_{b}^2 = \sigma_{b,\rm\ obs}^2  +  \left(\sum_{i} a_i\, \sigma_{b,i,\ \rm model}\right)^2
\end{equation}
where $a_i$ is the associated coefficient ($\aagb$, $\yo$, $\zcc$) for each yield process.
...
\strike{$\sigma_{\rm sys}$ represents a systematic uncertainty.
We consider models with both $\sigma_{\rm sys} = 0$ and 0.05 dex. While approximate, $0.05$ dex is similar to typical mean deviations between APOGEE and the C and Mg abundances in other surveys}\citep{hegedhus+2023, jonsson+2018}. \strike{Discrepancies between APOGEE's reported abundance uncertainties and other measurements of dispersion indicate a similar level of systematic error }\citep{mead+2024}.\strike{ While systematic uncertainties are unlikely to be well-described by an independent, uncorrelated, constant-width Gaussian distribution, we can nonetheless use this parameter as an approximation of the likely uncertainties of our results.}

We adopt the following priors for each model parameter: $\aagb \sim N(1, 1)$, $\zcc \sim N(0, 10^{-3})$, and \strike{$\yl$} $\add{\yo-\zcc} \sim N(2\times10^{-3}, 1\times10^{-3})$, where $N(\mu, \sigma)$ is a Gaussian distribution with mean $\mu$ and standard deviation $\sigma$. (For the doubled yields and outflows model, the priors are correspondingly doubled). 
...
\strike{By including the $\sigma_{\rm sys}$ parameter, the posterior distributions of $\fagb$, $\aagb$, and CC yield parameters all widen without changing our conclusions. Effectively, $\sigma_{\rm sys}$ reduces the weight of abundance differences between bins. Systematic errors likely dominate the uncertainty budget in this analysis.}
...
Fig.~\ref{fig:mcmc_ytot} shows the posterior distributions of our MCMC fit to the C yield models\strike{, described in Section~6, }assuming the \fruity{} AGB table. Each parameter is relatively well-constrained. The most prominent degeneracy, between \replace{$\yl$}{$\yo$} and $\aagb$, represents the constraint on the equilibrium abundance of C near solar. \strike{Another clear correlation, between \replace{$\yl$} and $\zcc$, is related to the solar yield constraint as well, allowing a narrow range of $\zcc$ values while retaining the solar $\yo = \yl + \zcc$ yield.}
...
\begin{table*}
    \caption{
        Best-fit parameters (medians and 16-84th percentile ranges) from our MCMC fit for each model discussed\strike{ (see Section 6)}.
        The columns are:  the model name, best-fitting log posterior probability $\log p$, AGB scaling $\aagb$, CCSN low-metallicity yield $\yl$, the CCSN yield slope $\zcc$, the AGB fraction $\fagb$, and the total C yield at solar metallicity $\Yct$. \note{parameters updated to new model, new last model added and removing $\sigma$ models for simplicity}
    }
...
\renewcommand{\baselinestretch}{1.2}
    ...
    \hline
    model            %& $\chi2$  
    & $\log p$ & $\aagb$ & $\yo/10^{-4}$ & $\zcc/10^{-4}$ &  $\fagb$ & $\Yct/10^{-4}$\\    %
\hline
\fruity          &  -536.38 & $2.15\pm0.04$  &  $19.75\pm0.15$  &  $8.6\pm0.1$  &  $0.253\pm0.005$  &  $26.42\pm0.04$\\ 
\aton            &  -657.17 & $1.53\pm0.03$  &  $25.1\pm0.06$  &  $14.79\pm0.2$  &  $-(5.5\pm0.1)\times10^{-4}$  &  $25.09\pm0.06$\\ 
\monash          &  -871.48 & $2.05\pm0.05$  &  $19.88\pm0.2$  &  $15.9\pm0.3$  &  $0.234\pm0.006$  &  $25.95\pm0.06$\\ 
\nugrid          &  -847.15 & $0.827\pm0.016$  &  $19.73\pm0.16$  &  $4.7\pm0.1$  &  $0.279\pm0.006$  &  $27.38\pm0.03$\\ 
\fruity\ shifted    &  -307.60 & $1.48\pm0.02$  &  $22.11\pm0.09$  &  $5.29\pm0.09$  &  $0.194\pm0.003$  &  $27.44\pm0.03$\\ 
doubled yields (and $\eta$) &  -551.30 & $4.26\pm0.08$  &  $39.6\pm0.3$  &  $16.95\pm0.19$  &  $0.251\pm0.005$  &  $52.84\pm0.08$\\ 
lateburst        &  -386.87 & $2.45\pm0.05$  &  $18.55\pm0.2$  &  $7.41\pm0.14$  &  $0.291\pm0.007$  &  $26.16\pm0.05$\\ 
twoinfall        &  -436.15 & $1.99\pm0.03$  &  $20.29\pm0.12$  &  $8.26\pm0.1$  &  $0.233\pm0.004$  &  $26.47\pm0.04$\\ 
higher SN Ia     &  -392.76 & $3.86\pm0.06$  &  $13.2\pm0.2$  &  $12.09\pm0.13$  &  $0.477\pm0.009$  &  $25.17\pm0.05$\\ 
higher SN Ia \& \fruity\ shifted &  -283.12 & $1.79\pm0.03$  &  $20.6\pm0.1$  &  $5.96\pm0.09$  &  $0.238\pm0.003$  &  $27.02\pm0.03$\\
\hline
\end{tabular}
...
$\aagb$ represents the scale factor for the AGB yields, and \replace{$\yl$}{$\yo$}, $\zcc$ represent the constant and linear components of the CCSN yield. 
}
...
\caption[]{
\red{
The population-averaged yield ratio, $\Yct / \Ymg$, based on our MCMC fits to the [C/Mg]-[Mg/H]-[Mg/Fe] relation. We also show individual
samples from the FRUITY MCMC model
}
}
\label{fig:mcmc_ytot}
\end{figure}
...
\section{Gas-Phase Abundances}\label{sec:extra_gas}
\note{Old section 7 moved here.}
...
As \replace{a final}{an additional} test of our model, we compare the predictions against C abundances
...
While we used Mg as our representative alpha element in our APOGEE sample, we shift focus to O in this \replace{Section}{Appendix} because O is more readily observed in the gas phase.
...
One possible remedy would be to decrease our assumed minimum CCSN yield $y_{\rm low} = \yo - \zcc$, which would lower the [C/O] plateau and improve agreement with observations. A lower $y_{\rm low}$ also requires a higher $\fagb{}$ to reach the observed [C/Mg] near solar metallicity. However, a high $\fagb{}$ model would be in tension with the subgiant abundance trends (see Fig.~\ref{fig:agb_yields}). 
