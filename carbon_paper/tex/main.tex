...
\begin{equation}
%y_{X}^{\rm CC} = \frac{ \int_{l_{\rm CC}}^{u} (E(M) M_{X\rm, SN} + w_X - Z_X\,(M-M_{\rm rem}) )\frac{dN}{dM}\ dM }{ \int_{l}^{u} M\,\frac{dN}{dM}\ dM }
...
\end{equation}
Similarly, we represent the mean and variance of the observations in each bin with $\bar Z_{b, \rm obs}$ and $\sigma^2_{b, \rm obs}$.  
...
\end{align}
\end{subequations}
We run the MCMC using Turing.jl with a No-U-turn sampler (acceptance rate of 0.65), combining 8 chains with 10,000 steps each. \dbnote{todo: increase at least for fiducial}.
