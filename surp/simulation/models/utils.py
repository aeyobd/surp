...
class double_exponential:
	r"""
	A double exponentail ment to replicate a two infall model.
	"""

	def __init__(self, norm=1, tau1=2, tau2=1, A21=3.47, t1=4.1, t2=13.2):
		self.norm = norm
		self.tau1 = tau1
		self.tau2 = tau2
		self.A21 = A21
	...
	def __call__(self, time):
		A = 1 / (self.tau1 * (1 - m.exp(-self.t1/self.tau1)))
		B = self.A21 / (self.tau2 * (1 - m.exp(-(self.t2-self.t1)/self.tau2)))
		B *= (time > self.t1)

		return self.norm * (
			A * m.exp(-time/self.tau1) + 
			B * m.exp((-time+self.t1)/self.tau2)
                )


class triple_exponential(double_exponential):
	r"""
	A tripple exponential extending the double exponentail
	"""

	def __init__(self, A32=0.4, tau3=0.15, t2=11, t3=13.2, **kwargs):
		super().__init__(t2=t2, **kwargs)
		self.tau3 = tau3
		self.A32 = A32
		self.t2 = t2
		self.t3 = t3
	...
	def __call__(self, time):
		C = self.A32  / (self.tau3 * (1 -
			m.exp(-(self.t3-self.t2)/self.tau3)
			))
		C *= (time > self.t2)

		return  self.norm * (
			super().__call__(time)
			+ C * m.exp(-(time-self.t2)/self.tau3)
			)
