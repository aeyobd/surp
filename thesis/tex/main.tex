...
\acsetup{
    ...
    templates/colspec = {p{0.2\linewidth} p{0.8\linewidth} },
    pages/display = none,
    format/short = {\sc}, 
}
...
\DeclareAcronym{cc}{
    short = ccsn,
    long = core collapse supernova,
    short-plural = e,
    long-plural = e,
    extra = {massive star explosions}
}
\DeclareAcronym{ia}{
    short = sne ia,
    long = supernovae type Ia,
    extra = {exploding white dwarfs}
}
...
\DeclareAcronym{gce}{
    short = gce,
    long = galactic chemical evolution
}

\DeclareAcronym{dla}{
    short = {dla},
    long = {damped Lyman-alpha},
    short-plural = s,
    long-plural = \ systems,
}

\DeclareAcronym{apogee}{
    short = {apogee},
    long = {Apache Point Observatory Galactic Evolution Experiment},
    extra = {a large optical spectroscopic survey of Milky Way stars}
}
...
\DeclareAcronym{sfh}{
    short = sfh,
    long = star formation history,
}
\DeclareAcronym{ssp}{
    short = ssp,
    long = single stellar population,
}
\DeclareAcronym{imf}{
    short = imf,
...
\newcommand{\cc}{\ac{cc}}
\newcommand{\agb}{\ac{agb}}
\newcommand{\ia}{\ac{ia}}

\newcommand{\sfh}{\ac{sfh}}
\newcommand{\ssp}{\ac{ssp}}
\newcommand{\imf}{\ac{imf}}
\newcommand{\gce}{\ac{gce}}
\newcommand{\apogee}{\ac{apogee}}
...
\Ac{gce} aims to understand the chemical and star formation histories of galaxies. At the core of \ac{gce} models are stellar yields---the amount of each chemical element stars produce (or destroy). Each element is produced in different amounts by different stars, leaving traces of the Galaxy's evolutionary history. Through \textit{Galactic Archeology}, we can reconstruct the history of our galaxy by investigating clues left behind in stars.

Here, I aim to understand C enrichment---where it is produced and how its abundances evolve over time. C is unique nucleosynthetically, one of few light elements (along with N) produced in \ac{agb} stars \citep[e.g.][]{jennifer19, KL14}. C and N are well-studied elements since they are easy to observe in stellar spectra, even at the lowest metallicities \cite[e.g.][]{fabbian+09, nissen+14, lambert81, laird85, lambert86}. Additionally, C and N are used as age indicators in red giant branch stars \citep{martig16, MG15, hasselquist19, vincenzo+21}.

Gas-phase [C/O] ratios---observed in very low metallicity, high redshift \acp{dla}---decrease with increasing [O/H] \citep{FN15, cooke+17}. Then, [C/O] increases above $\rm [O/H] \approx -1$ (\citealt{berg+19}; see discussion in \S\ref{sec:gas}).
While we know C is produced in \ac{agb} stars and \acp{cc}, we still have a limited understanding of the magnitude and metallicity-dependence of each process.
...
Theoretical models of stellar nucleosynthesis provide the starting point of this investigation. I focus on three primary nucleosynthetic pathways: \acf{agb} stars, \acf{cc}, and type Ia supernovae \acf{ia}. Each process has unique timescales and yields, traceable through the tools of \gce. I compare C, produced in \cc{} and \ac{agb} stars, to Mg and Fe as tracers of \cc{} and \acf{ia} espectively.

When a \ssp~forms, \cc{} are the first enrichment event. \cc{}  explode within $\lesssim 40$ Myr, providing light elements (C, O, and Mg) and heavier elements (Fe and beyond). Metallicity-independent yields from \cc{} are the only statistically significant source of O and Mg ($\alpha$ elements). Next, low-mass stars begin to reach the end of their lives. By shedding their outer layers, \ac{agb} stars are important sources of C, N, and neutron capture elements.  Finally, white dwarfs explode, releasing Fe and other iron-peak elements.
...
Although per-star yields are necessary to compute \ac{agb} star enrichment rates in \gce{}  models, \imf-averaged\footnote{IMF: Initial Mass Function} yields are useful in interpreting their predictions. For a yield $y$ from a star of mass M and initial metallicity $Z$, the \imf-averaged yield is given by 
\begin{equation} \label{eq:imf-yield}
    ...
    \frac{dN}{dM}  \ dM
\end{equation}
where ${dN}/{dM}$ is the normalized \imf, and $M_{\rm min}$ and $M_{\rm max}$ are the minimum and maximum mass of stars, which I take to be $0.08 M_{\sun}$ and $100 M_{\sun}$ respectively. For \ac{agb} stars, the yields are truncated above 8 $M_{\odot}$. 
...
Also following \citet{james+21, james+23}, I take the \ac{ia} delay time distribution to be a
...
An \ac{agb} star is a low mass ($\lesssim 8 M_{\sun}$) star during its final phase of evolution.  In an \ac{agb} star, two competing processes determine the outcome of C production: third dredge up and hot bottom burning.  Third dredge up accompanies thermal pulses in \ac{agb} stars. During a third dredge up event, material from the CO core is mixed with material exposed to partial He-shell burning, increasing C and O abundances in the envelope \citep{KL14}. If this envelope is lost during the \ac{agb} phase, then C yields are enhanced.
...
As a result, when both third dredge up and Hot bottom burning occur, \isotope[12]{C} yields are lowered (see discussion in \citealt{james+23} and \citealt{ventura+13}).

In this work, I explore four different sets of \ac{agb} star yield tables from literature, providing well-sampled grids in metallicity and mass for use in chemical evolution models. 
\begin{itemize}
...
\end{itemize}
I use C11 table, amplified by a factor of 2.9, as the fiducial \ac{agb} yield.

Fig.~\ref{fig:y_agb} compares the \ac{agb} C yield $\tilde{y}_\text{C}^\text{AGB}(M, Z)$ for each model I consider. 
...
Most models agree on the qualitative shape of the net fractional \ac{agb} C yield---% 
stars with masses between \about{2~M_\odot} and \about{4~M_\odot} have the highest fractional C yields, with the mass of the peak increasing and overall yields decreasing with increasing $Z$.  High mass, high $Z$ stars destroy \isotope[12]{C} because they experience both third dredge up and hot bottom burning, but the latter is much more efficient.

In Fig.~\ref{fig:agb-ssp}, I plot the total production of C by \ac{agb} stars in a \ssp{} at an age $t$. 
...
As the mass range $2\lesssim M \lesssim 4$ is most important for C production, about half of C production occurs before \about{1} Gyr, similar to \ac{ia} Fe. 
...
\begin{figure}
    \centering
        ...
        \caption[Low mass C yields]{The net fractional \ac{agb} C yield  plotted as a function of initial stellar mass $M$ and color-coded according to metallicity. The black dashed line shows $\tilde{y}=0$ for reference. Each panel represents yields from one of four \ac{agb} models: C11, K10, V13, K16 (see \S \ref{sec:agb}). }
        \label{fig:y_agb}
\end{figure}
\begin{figure}
    ...
    C production by \ac{agb} stars as a function of \ssp{} age, normalized to the total mass $M_{\rm tot}$ produced at $t=10$ Gyr. Left: The four \ac{agb} yield models from literature at solar metallicity (C11, K10, V13, or K16). The delay time distribution of type Ia supernovae ($\propto t^{-1.1}$) is plotted as a dashed black line for comparison. Right: The C11 \ac{agb} model at different metallicities. }
    \label{fig:agb-ssp}
\end{figure}


Fig.~\ref{fig:yagb-z} shows \imf-averaged C yields for each \ac{agb} model as a function of metallicity $Z$.
...
\begin{figure}
    ...
    \caption[Low mass yield metallicity dependence]{The net fractional \imf-weighted \ac{agb} C yield $\Ycagb$ as a function of metallicity for each of our \ac{agb} yield models.
    }
    \label{fig:yagb-z}
\end{figure}
...
Fig.~\ref{fig:y_cc} shows the C11 model for comparison on the top. Especially at $Z\approx Z_\odot$, most \cc{} models dominate \ac{agb} C production. I will later also show empirically that \cc{} should dominate C production. 
...
Once again, different \acp{cc} models span a large range in [C/Mg]. 
I chose to instead parameterize $\Ycc$ to enable agreement with observations, as most \acp{cc} models fail to achieve near-solar [C/Mg].
...
\acp{cc} models also do not reach [O/Mg] due to overproduction of O, or underproduction of Mg, or both \citep{emily+21}. Here, I assume ${\rm [O/Mg]} = 0$, which is not compatible with \acp{cc} models but is consistent with \apogee{} observations \citep{weinberg+19, weinberg+22}.
...
\begin{figure}[htp]
    ...
    \caption[Massive star C yields]{
        ...
        \textbf{Top:} The \imf-weighted \acp{cc} yield of C as a function of metallicity.
        \textbf{Bottom:} The \acp{cc} [C/Mg] abundance ratio, defined in Eq.~\ref{eq:c_mg_cc}. The black line represents the C yield of the fiducial model,
...
\end{equation}
where the first term on the right-hand side describes \acp{cc} enrichment. 
...
While Equation \ref{eq:z_eq} can be applied to \acp{cc} C, the delayed nature of \ac{agb} C can complicate the expression. Instead, I use an effective yield, which can be expressed as an integral over the DTD,
\begin{equation}
...
This argument can also be inverted to infer  yields from abundance ratio trends. To the extent that observed C and Mg trends reflect the equilibrium abundances in different Galactic regions, we can infer the \acp{cc} yield given an assumed \ac{agb} star yield (or vice versa). Inferring $\Ycc$ from $\Ycagb$, 
\begin{equation}
...
In \S\ref{sec:f-z-models}, I will show that none of the four \ac{agb} yield sets (C11, K10, V13, K16) produce enough C relative to this $\Ycc$ value. So, I introduce normalization factors, $\alpha_{\rm AGB}$, and $\alpha_{\rm CC}$ which denote a multiplicitave scaling of $\Ycc$ and $\Ycagb$ respectively. 
...
In \S\ref{sec:f-z-models}, I find that $\alpha_{\rm AGB}\approx 2.9$ for the C11 yield model. Using this \ac{agb} yield, I can now use Eq.~\ref{eq:y_c_eq} to find an estimate of $\Ycc$ as a function of metallicity. I show the values of $\Ycc$ I obtain from the subgiant sample in Fig.~\ref{fig:analytic}. From regression analysis, I suggest here that
\begin{equation}
...
\begin{figure}
    ...
    \caption[Reverse fit yields]{Inferred massive star C yields as a function of metallicity. I assume equilibrium and $3\times {\rm C11}$ \ac{agb} yields (orange curve, see discussion in Section \ref{sec:equilibrium}). The binned medians and median absolute deviation (divided by $\sqrt{N}$) are blue points.
    }
    \label{fig:analytic}
\end{figure}
...
The fiducial model has the following qualitative characteristics of its C yields: (a) C is predominantly produced in \acp{cc}, (b) \acp{cc} produce more C at higher metallicities, and (c) \ac{agb} stars produce less carbon at higher metallicities. Our fiducial model uses the C11 \ac{agb} yield tables (see \S\ref{sec:agb}, and Table \ref{tab:fiducial_mod}), and I amplify the C11 yields by a factor of 2.9 such that \ac{agb} stars account for 20\% of carbon at $Z=Z_\odot$. 

I show the time evolution tracks of the fiducial model in Fig.~\ref{fig:c_evo} for \caah~ and \caafe~. Comparing [C/Mg] against [Mg/Fe] enables us to see the late-time evolution of C more clearly. Because of the extended DTD of \ac{ia} enrichment, [Fe/H] takes longer to reach equilibrium as half of Fe production comes from type-Ia supernovae, and the late time evolution is not as clustered as [Mg/H].
The evolution proceeds as follows.
Initially, \acp{cc} dominate production. As $\Ycc$ has strong metallicity dependence, [C/Mg] increases with time. Shortly thereafter, \ac{agb} stars contribute delayed C, increasing [C/Mg] even more steeply. As Mg begins to reach equilibrium, the [C/Mg] ratio plateaus as C approaches equilibrium. Finally, as $\Ycagb$ decreases or even becomes negative with higher metallicity, the [C/Mg] abundance may decline slightly. 
This is even more evident in the lower panel of Fig.~\ref{fig:c_evo}. While the \caah~trend reaches equilibrium at \about{5} Gyr, the \caafe~trend continues to evolve even until the present day, exposing the effect of the relative delay times of \ac{agb} stars and \ac{ia}.
...
Fig.~\ref{fig:agb_sims} compares the K10, K16, C11, and V13 yield models. Here, I use the published, unscaled yield tables. As the highest \ac{agb} yield at solar, K10 is only $\Ycagb=0.000585$, $f_{\rm AGB} \leq 0.12$ for all models. Massive stars dominate C production with unscaled yields.  While it is easy to modify the \ac{agb} yields in my model, their small contribution reduces the effect. I leave a more detailed discussion of the effects of \ac{agb} models for Appendix \ref{sec:alt_agb} and use C11 yields hereafter.

\begin{figure}
\centering
\includegraphics{oob_agb.pdf}
\caption[AGB GCE Models]{
    Stellar abundance trends in our model, assuming metallicity independent $\Ycc$. Colored lines quantify the median [C/Mg] in bins of [Mg/H] for our four \ac{agb} yield models from the literature (see \ref{sec:agb}). Black points and grey dashes represent the median and standard deviations in the \citetjack~sample. In the right panel, I show the trends only for stars where $-0.15\leq {\rm [Mg/H]}\leq -0.05$.
}
\label{fig:agb_sims}
\end{figure}
...
Next, I investigate adjustments to the \ac{agb} yield fraction $f_{\rm AGB}$ and the \acp{cc} metallicity dependence $\zeta$ in Fig.~\ref{fig:beta_f}. On the top panel of Fig.~\ref{fig:beta_f}, I plot models with varying $\Ycc$ metallicity dependence. As discussed in \S\ref{sec:equilibrium}, the \caah~trend is approximated by equilibrium, so the trends of these have steeper \caah~corresponding to steeper metallicity dependence. However, \caafe~is minimally affected by these changes since \acp{cc} occure on much shorter timescales than \ac{ia} and \ac{agb} enrichment.

Now, I vary the proportion of \ac{agb} contribution.  The \ac{agb} C production fraction is defined as
\begin{equation}
...
In the bottom panel of Fig.~\ref{fig:beta_f}, I plot three models with different \ac{agb} fractions while using C11 yields.  The \caafe~relationship is set by $f_{\rm AGB}$ because a specific amount of C must be released at a delayed time in order to match the \ac{ia} production of Fe and increase [C/Mg] as [Mg/Fe] decreases to reproduce the data.
...
One source of theoretical uncertainty in this result is that the \ac{ia} yield and DTD have their own uncertainties. I discuss variations in $y_{\rm Fe}^{\rm Ia}$ in Appendix \ref{sec:alt_agb}, and find that the qualitative conclusions are largely unaffected. I therefore focus on the choices of $y_{\rm Fe}^{\rm Ia} = 0.00214$ and a $\propto t^{-1.1}$ DTD choices from the fiducial model here.
In short, the scaling of the trend and metallicity dependence of C (as seen in
the \caah\ trend) gives information on the total C yield and the behavior of \acp{cc} (as the dominating producer of C), the \caafe\ trend exposes the delayed effect of C from \ac{agb} contribution.

\begin{table}
	\centering
    \caption[Low mass star solar C yields]{For each \ac{agb} yield set, the \imf-weighted C \ac{agb} yield at solar metallicity, along with the multiplicative factor reaches an \ac{agb} contribution of 20\%.}
	\label{tab:alpha_agb}
...
As an additional test of the model, I next compare the gas-phase predictions against gas-phase C data. In Fig.~\ref{fig:gas_phase}, I plot our fiducial model's gas phase predictions compared to observations of MW and extragalactic HII regions, halo stars, and \acf{dla} systems. The model is broadly consistent with observations, where the model at $t=2$ Gyr approximates the slope of dwarf galaxies and halo stars. The increase of C/O at late times is also consistent with the high C/O abundances measured in extragalactic HII regions. 
...
As \ac{agb} stars do have a significant delay, these stars cannot explain the increase in C yields at low metallicity as the timescales to reach $\rm [O/H]\sim -1$ is faster than this enrichment can occur. So, enhanced carbon due to (rotating) massive population III stars at low metallicities explains this trend.
...
\caption[Gas phase abundances]{Gas-phase C abundances. We plot our model at $t=2$Gyr and present day as thick solid lines. Points represent measurements in 
    ...
    \acf{dla} systems \citep[blue triangles;][]{cooke+17} 
...
Though \acp{cc} C yields are poorly understood, I adopt an equilibrium approximation and determine a functional form that approximately matches trends in \apogee{} subgiants and is consistant with massive star nucleosynthesis models. Variations of the metallicity dependence of this \acp{cc} yield affect trends in \caah~but do not affect trends in \caafe\ when taking a slice in [Mg/H]. As all theoretical \ac{agb} C yields decrease with metallicity, increasing the \ac{agb} fraction causes the \caah\ trend to flatten. However, the \caafe\ trend is sensitive to the \ac{agb} fraction. 
From this, I estimate that \ac{agb} stars contribute \about{20\%} of total C abundance at solar metallicity. The remaining \about{80\%} of C comes from massive stars with a metallicity dependent yield of $\Ycc/\Yoc=1.51 + 0.54 (Z/Z_\odot)$. 
...
As I focus on constraining relative yields, I neglect O and Mg yield variations in the main text. There is substantial variation in Mg predictions (see Fig.~\ref{fig:y_mg}). Most models predict relatively flat Mg trends with metallicity (even with rotating models from \citealt{LC18}). However, the variation is significant and my adopted $\Yoc$ yield is much higher than most models. This is a known problem (see \citet{emily+21}). \acsp{cc} models overpredict O or underpredict Mg, and the resolution is still unknown. 
My results are, however, independent of whether the \acs{cc} element is O or Mg.

While I focus the main discussion of the paper on the C11 model, other \ac{agb} models can impact abundance trends when $f_{\rm AGB}$ becomes larger.
Here, I briefly explore the effects of variations in \ac{agb} models that affect abundance trend predictions when these models are amplified to match observational [C/MG]-[Mg/Fe] trends. 
...
\begin{figure}
    ...
    \caption[Adjusting type Ia iron]{Similar to Fig.~\ref{fig:agb_sims} except with variations of the \ac{ia} iron fraction}
    \label{fig:fe_ia}
\end{figure}
